 ...
 */
final class VTraceHelper
{
    private static ?VTracer $tracer = null;
    ...
    {
        self::$tracer = VTracer::getInstance();
    ...
    public static function traceAwait(Generator $generator, string $operationName = 'unknown', array $tags = []): Generator
    {
        ...
        try {
            ...
            $result = yield from VOsaka::await($generator);
    ...
    /**
     * Trace VOsaka::defer operations
     */
    public static function traceDefer(Closure $callback, string $operationName = 'deferred_callback', array $tags = []): void
    {
        if (!self::$tracer || !self::$tracer->isEnabled()) {
            VOsaka::defer($callback);
            return;
        }
